function [storms_stats]= climada_calculate_storms_stats_inregion(tc_track,inregion)
% ---------------------------------------------
...
for season_i = 1:season_count
    ...
    for index_i = 1:length(season_index)
        % in region 
        if 1-isempty(inregion)
            inflag = any(tc_track(season_index(index_i)).lon <=max(inregion.lonlim) & ...
                          tc_track(season_index(index_i)).lon >=min(inregion.lonlim) & ...
                          tc_track(season_index(index_i)).lat <=max(inregion.latlim) & ...
                          tc_track(season_index(index_i)).lat >=min(inregion.latlim) );
            if inflag==0, 
                continue, end 
        end
...
%%
storms_stats.ACE          = ACE;
storms_stats.TS_count     = TS_count;
storms_stats.Hur_count    = Hur_count;
storms_stats.Hur3_count   = Hur3_count;
storms_stats.counts       = counts; 
storms_stats.counts_sorted= counts_sorted; 
storms_stats.counts_max   = counts_max; 
storms_stats.counts       = counts; 
storms_stats.counts_ori   = counts_ori; 
