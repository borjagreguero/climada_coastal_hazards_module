function res=climada_damagefunctions_plot_coastal(entity,unique_ID_sel)
% climada
% NAME:
...
unique_IDs=unique(unique_ID,'stable');
[unique_Names,ia]=unique(damagefunctions.Description,'stable');
unique_percentage=damagefunctions.Percentage(ia);
...
%         title(unique_IDs{ID_i});
        ...
        % MODIFICATIONS FOR MEXICO 
        if unique_percentage(ID_i)~=1
            set(get(gcf,'CurrentAxes'),'YLim',[0 ceil(max(res.MDD))]); % in dollars 
            ylabel('MDR ($)')
        end
        title(unique_Names{ID_i});
        % -- 
