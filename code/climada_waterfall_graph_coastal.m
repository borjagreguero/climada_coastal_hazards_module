...
if ~exist('type_hazard','var'), type_hazard = 'flood'; end
if ~exist('y_axis_limits','var'), y_axis_limits = []; end

if isempty(xlabels_descrpt)
    xlabels_descrpt={{'Present';'expected damage'}, {'Incremental increase';'from economic growth';'(no climate)'},...
                     {'Incremental increase';'from climate change'},{'Total Future Risk'}}; 
end
%---------------------------------------------
...
damage = damage./unit_scale; 
digits  = log10(unit_scale);

% TIV of portfolio
% % % TIV_nr = round(unique([EDS(:).Value])*10^-digits);
% % % N      = -abs(floor(log10(max(TIV_nr)))-1);
% % % TIV_nr = round(TIV_nr*10^N)/10^N;  
TIV_nr = round(unique([EDS(:).Value]*10));
TIV_nr = round(TIV_nr./10^digits)./10
...
switch type_hazard
    ...
    case 'flood'
        ...
        color_     = [227 236 208;...   %today
                      ...
                      120 120 120]/256; %dotted line]/255;
        color_(1:4,:) = brighten(color_(1:4,:),-0.1);
end        
% % green color scheme
% color_     = [227 236 208;...   %today
%               194 214 154;...   %eco 
...
if 1-isempty(y_axis_limits)
    ylim(y_axis_limits) 
end

% ylabel(['damage amount \cdot 10^{', int2str(digits) '}'],'fontsize',fontsize_)
ylabel(['damage amount \cdot ',units],'fontsize',fontsize_)
...
% dED3 = 0.10;
dED3 = stretch+0.07;
climada_arrow_2015 ([2+dED2 damage(2)], [2+dED2 damage(3)], 40, 10, 30,'width',1.5,'Length',10, 'BaseAngle',90, 'EdgeColor','none', 'FaceColor',[0.5 0.5 0.5]);
...
%arrow cc
climada_arrow_2015 ([3+dED2 damage(3)], [3+dED2 damage(4)], 40, 10, 30,'width',1.5,'Length',10, 'BaseAngle',90, 'EdgeColor','none', 'FaceColor',[0.5 0.5 0.5]);
text (3+dED3, damage(3)+diff(damage(3:4))*0.5, ['+' int2str((damage(4)-damage(3))/damage(2)*100) '%'], ...
      'color',[0. 0. 0.],'HorizontalAlignment','left','VerticalAlignment','middle','fontsize',fontsize_-1);
  
%arrow total
climada_arrow_2015 ([4 damage(2)], [4 damage(4)], 40, 10, 30,'width',1.5,'Length',10, 'BaseAngle',90, 'EdgeColor','none', 'FaceColor',[256 256 256]/256);
...
if digits~=0
    textstr_TIV = sprintf(['Total assets: %.3g 10^%d %5s'], TIV_nr, digits,units);
else
    textstr_TIV = sprintf(['Total assets: %.3g %5s'], TIV_nr, units);
end
if 1-isempty(y_axis_limits) 
    text(1-stretch, y_axis_limits(2)*0.99,textstr, 'color','k','HorizontalAlignment','left','VerticalAlignment','top','FontWeight','bold','fontsize',fontsize_);
    text(1-stretch, y_axis_limits(2)*0.97,textstr_TIV, 'color','k','HorizontalAlignment','left','VerticalAlignment','top','FontWeight','normal','fontsize',fontsize_2);
else
...
if 1-isempty(y_axis_limits) 
%     set(gca,'xtick',[1:4],'xticklabel',{'Present',sprintf('Incremental increase\n from economic growth\n (no climate)'),...
%         'Incremental increase \n from climate change','Total Future Risk'}) 
    set(gca,'xtick',[1:4]) 
    dy = (y_axis_limits(2)-y_axis_limits(1))/20; 
    
    text(1, y_axis_limits(1)-dy, xlabels_descrpt{1}, 'FontSize',10, 'HorizontalAlignment','Center');
    text(2, y_axis_limits(1)-dy, xlabels_descrpt{2}, 'FontSize', 8, 'HorizontalAlignment', 'Center');
    text(3, y_axis_limits(1)-dy, xlabels_descrpt{3}, 'FontSize', 8, 'HorizontalAlignment','Center');
    text(4, y_axis_limits(1)-dy, xlabels_descrpt{4}, 'FontSize', 10, 'HorizontalAlignment', 'Center');
else
    text(1-stretch, damage(1)-max(damage)*0.02, {['Present'];'expected damage'}, 'color','k','HorizontalAlignment','left','VerticalAlignment','top','fontsize',fontsize_2);
    text(2-stretch, damage(1)-max(damage)*0.02, {'Incremental increase';'from economic growth';'(no climate)'},          'color','k','HorizontalAlignment','left','VerticalAlignment','top','fontsize',fontsize_2);
    text(3-stretch, damage(1)-max(damage)*0.02, {'Incremental increase';'from climate change'},                                 'color','k','HorizontalAlignment','left','VerticalAlignment','top','fontsize',fontsize_2);
    text(4-stretch, damage(1)-max(damage)*0.02, 'Total Future Risk','color','k','HorizontalAlignment','left','VerticalAlignment','top','fontsize',fontsize_2);
end

%Legend
% L = legend(h,legend_str(index),'location','NorthOutside','fontsize',fontsize_2);
L = legend(h,legend_str(index),'location','South','fontsize',fontsize_2);
set(L,'Box', 'off')
