function res=climada_tc_windfield_ACE(tc_track,centroids,equal_timestep,silent_mode,check_plot)
...
res.ACE_track_i  =  zeros(1,centroid_count);
...
for centroid_ii=1:centroid_count % now loop over all valid centroids
    ...
    if D<5*R % focus on the radius that really has an impact
        ...
        v_35kn = S./1.15*1.61; % km/h to kn 
        res.ACE_track_i(centroid_i)   = sum(v_35kn.^2)*10^-4;
