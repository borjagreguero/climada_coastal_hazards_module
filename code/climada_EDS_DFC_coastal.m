function [fig,legend_str,return_period,sorted_damage] = climada_EDS_DFC_coastal(EDS,EDS_comparison,Percentage_Of_Value_Flag,plot_loglog)
% climada
% NAME:
%   climada_EDS_DFC
% PURPOSE:
...
for EDS_i=1:length(EDS)
    if isfield(EDS(EDS_i),'R_fit') % for EDS struct  - that includes stats 
        return_period           = EDS(EDS_i).R_fit;
        sorted_damage           = EDS(EDS_i).damage_fit;
        if Percentage_Of_Value_Flag
            sorted_damage = EDS(EDS_i).damage_fit/EDS(EDS_i).Value.*100;
        end
    else
...
if ~isempty(legend_str),legend(legend_str,'Location','Southeast');end % add legend
