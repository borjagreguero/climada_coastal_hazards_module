function hazard = climada_tc_ACE_set(tc_track, hazard_set_file, centroids)
...
ACE          = zeros(season_count,numel(centroids.lon));
TS_count_all = zeros(season_count,1);
TS_count     = zeros(season_count,numel(centroids.lon));
Hur_count    = zeros(season_count,numel(centroids.lon));
Hur3_count   = zeros(season_count,numel(centroids.lon));
for season_i = 1:season_count
    season_index = find( season_storm == seasons(season_i));
    TS_count_all(season_i) = length(season_index);
    for index_i = 1:length(season_index)
        ACE(season_i,:) = ACE(season_i,:) + hazard.ACE(season_index(index_i),:); % accumulate for all centroids 
        
        col = find(hazard.ACE(season_index(index_i),:)>0); 
        TS_count(season_i,col)= TS_count(season_i,col) +1; 
        if tc_track(season_index(index_i)).category>=1
            Hur_count(season_i,col)  =  Hur_count(season_i,col) +1;
        end
        if tc_track(season_index(index_i)).category>=3
            Hur3_count(season_i,col) =  Hur3_count(season_i,col) +1;
        end
    end
end
hazard.ACE      = ACE'; 
hazard.TS_count = TS_count'; 
hazard.TS_count_all = TS_count_all'; 
hazard.Hur_count = Hur_count'; 
hazard.Hur3_count= Hur3_count'; 
